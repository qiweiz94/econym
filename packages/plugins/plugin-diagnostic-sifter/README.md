...
- **`targetPath` rejects only a leading `-`** — this blocks the common option-injection shape (`--outDir=...`) but is not a general shell/argv sanitizer; the path is passed as one argv element to a spawned process, never shell-interpreted.
