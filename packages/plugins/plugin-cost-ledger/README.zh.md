...
账本将每条已提交的 `assistant/message` 用量记录归因到产生它的提供方/模型对（`message.source`），统计 token 明细（输入、输出、缓存读、缓存写、推理），并按费率表计价。每个事件按其自身时间戳计价，因此跨越高峰/非高峰时段的会话会按运行当时适用的费率对每一步计费。折叠是**派生的，从不存储**：工具每次调用都重新读取会话日志，因此 Harness 重启后会重新计算相同的数字，无需检查点。
...
- `pricing` — 按模型的费率，单位为**每百万 token 的美元**，叠加在内置目录表（DeepSeek Harness `opencode-go` 目录的已发布费率）之上。两者都不覆盖的模型会被计数，但成本显示为 `null`——绝不臆造估算值。费率条目可携带 `peak` 块（`{ input, output, cacheRead?, cacheWrite? }`）；存在时，高峰时段按这些费率计费，其余时段按基础费率计费。没有 `peak` 块的模型在所有时段都按基础费率计费。
- `peakHours` — 高峰费率适用的 UTC 小时窗口 `[start, end)`；默认采用 DeepSeek 已发布的时段（01:00-04:00 与 06:00-10:00 UTC）。设为空列表可禁用高峰计价。
...
- **高峰计价仅按时段** — DeepSeek 已发布的高峰时段固定于内置目录并可通过 `peakHours` 覆盖；会话中途费率变更会从该点起重新计价。缓存输入定价（DeepSeek V4 上缓存读约为未命中输入价的 3%）按事件的 `cacheReadTokens` 逐事件应用。
