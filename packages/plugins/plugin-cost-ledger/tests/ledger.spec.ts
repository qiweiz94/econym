...
describe('foldLedger', () => {
  it('aggregates two models with built-in pricing and sums a whole-ledger total', () => {
    ...
    // deepseek-v4-flash at builtin rates: (1M×0.14 + 1M×0.28 + 1M×0.0028) / 1M = $0.4228
    const flash = snapshot.models.find(m => m.model === 'deepseek-v4-flash')
    expect(flash?.estimatedCostUsd).toBe(0.4228)
    ...
    expect(snapshot.totals.estimatedCostUsd).toBeCloseTo(0.4228 + 0.875, 6)
