/**
 * Settlement of one ONE-SHOT subagent run into a foreground tool result.
 *
 * Inlined from the fork's `dsh-subagent/foreground-settlement` (PR #20) so the
 * standalone plugin works against the published `@deepseek-ai/dsh-subagent`
 * API, which does not export these helpers.
 */
